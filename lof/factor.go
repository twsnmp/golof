// Package lof is a Local Outlier Factor algorithm (`Markus M. Breunig) implementation.
package lof
...
// NewLOF : Constructor for LOF type. Check out ./samples.GetSamplesFromFloat64s()
...
// Train : Pre-compute distances between training samples and store their
// nearest neighbors in LOF.KNNs.
func (lof *LOF) Train(samples []ISample) error {
	if err := lof.checkSamples(samples); err != nil {
		return err
	}
...
// GetLOFs : Shortcut for getting LOF for many samples. See GetLOF() method.
...
// GetLOF : Returns LOF value for a sample; does lots of things, read the original
...
// Reset : This function resets the LOF.KNNs table to the state right after
...
func (lof *LOF) updateNNTable(sampleIndex int, mode string) error {
	bound := 0
	switch mode {
	case "train":
...
func (lof *LOF) checkSamples(samples []ISample) error {
	if len(samples) < lof.MinPts {
		return fmt.Errorf("not enaough samples")
	}
	return nil
}
